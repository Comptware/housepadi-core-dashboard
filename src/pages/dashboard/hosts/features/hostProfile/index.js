...
const UserBookingsProfile = observer(() => {
  ...
  const { handleFindHost, searchLoading } = HostStore;
  const path = findPath(location, "/dashboard/agents");

  // useEffect(() => {
  //   handleFindHost({ url: path.path, navigate, route: -1 });
  // }, []);
  ...
  return (
    <div className="flex flex-col justify-start items-start h-full w-full">
      <div className="flex ustify-start items-center space-x-3 h-fit  w-full bg-white p-4 border-b-1/2 border-grey-border">
        ...
        <h3 className="text-lg text-black regular-font">Agent Profile</h3>
      </div>
