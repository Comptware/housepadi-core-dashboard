...
const ChatSection = () => {
  ...
  const updateConversation = async (message, count) => {
    try {
      ...
      let payload = {
        ...
        unreadAgentChats: unreadAgentChats?.length + count,
        unreadUserChats: 0,
      };
      const payloadAlt = {
        unreadUserChats: 0,
  ...
  const updateUserReadStatus = async () => {
    const userChats = chats?.filter(
      (chat) => chat?.senderId === currentChat?.userId
    );
    const unreadUserChats = userChats?.filter((chat) => !chat?.isRead);
    try {
      const batch = writeBatch(db);
      for (let index = 0; index < unreadUserChats?.length; index++) {
        const currChatRef = doc(db, unreadUserChats[index]?.path);
            ...
            >
              <div className="w-fit h-fit">
                <div
                  ...
                  style={{
                    backgroundImage: `url(${
                      currentChat?.userImage || DEFAULT_AVATAR
                    })`,
            ...
            {chats?.map(({ message, dateTime, senderId, url, type }, i) => {
              ...
              return (
                <Chat
                  ...
                  title={isUser ? "you" : currentChat?.userName}
                  messages={[message]}
          ...
          {showProfileModal && (
            <UserProfileModal
              data={{
                ...
                image: currentChat?.userImage || DEFAULT_AVATAR,
                phone_number: currentChat?.userPhoneNumber,
