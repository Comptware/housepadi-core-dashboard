...
const MessagesHome = observer(() => {
  ...
  const showChatListSection =
    (width < 768 && !currentChat?.userId) || width >= 768;
  const showChatSection = (width < 768 && currentChat?.userId) || width >= 768;
  return (
    <div className="flex flex-col justify-start items-start h-full w-full">
      <div className="flex flex-col lg:flex-row justify-start items-start h-full w-full mb-24 relative">
        ...
        {showChatListSection && (
          <div className="flex flex-col w-full md:w-[40%] justify-start items-start h-full space-y-7">
        ...
        {showChatSection && (
          <div
            className={`flex flex-col w-full md:w-[60%] lg:w-[50%] fixed right-0 h-[calc(100%-120px)] bg-white justify-start items-center space-y-7 border-l-1/2 border-grey-border`}
          >
            <ChatSection />
          </div>
        )}
